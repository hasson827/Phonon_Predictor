...
def get_qpts(astruct, res=0.05, threshold=1e-7, symprec=1e-5, angle_tolerance=-1.0):
    """
    ...
    """
    struct = get_structure(astruct) # Get the structure data (cell, positions, atomic numbers) from the ASE object
    ...
    for i in range(len(point_list)-1):
        ...
        if i not in break_from:
            for j in range(dig):
                kvec = ((dig - j) * start + j * end) / dig  # Linear interpolation to get q-points
